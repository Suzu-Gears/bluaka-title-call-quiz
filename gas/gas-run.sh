...
CLASPRC="$(cygpath -m "$HOME")/.clasprc.json"
...
if [ -f "$CONFIG_FILE" ]; then
  WEBAPP_URL="${WEBAPP_URL:-$(python -c "import json; print(json.load(open('$CONFIG_FILE')).get('webappUrl',''))" 2>/dev/null)}"
  WEBAPP_DEPLOY_ID="${WEBAPP_DEPLOY_ID:-$(python -c "import json; print(json.load(open('$CONFIG_FILE')).get('webappDeployId',''))" 2>/dev/null)}"
  SCRIPT_ID="${SCRIPT_ID:-$(python -c "import json; print(json.load(open('$CONFIG_FILE')).get('scriptId',''))" 2>/dev/null)}"
fi
...
do_setup() {
  ...
  local script_id
  script_id=$(python -c "import json; print(json.load(open('$clasp_json'))['scriptId'])")
  ...
  local deploy_id
  deploy_id=$(echo "$resp" | python -c "import json,sys; print(json.load(sys.stdin).get('deploymentId',''))" 2>/dev/null)

  if [ -z "$deploy_id" ]; then
    echo "Error: Failed to create Web App deployment" >&2
    echo "$resp" | python -m json.tool 2>/dev/null >&2 || echo "$resp" >&2
    echo "" >&2
...
do_deploy() {
  ...
  # テンプレート用プロジェクトにも同じソースをpushする(Web Appデプロイは不要)
  if [ -f "$SCRIPT_DIR/template/.clasp.json" ]; then
    echo ""
    echo "=== template push ==="
    cp "$SCRIPT_DIR/spreadsheet-sync.gs" "$SCRIPT_DIR/appsscript.json" "$SCRIPT_DIR/template/"
    (cd "$SCRIPT_DIR/template" && clasp push --force)
  fi
  ...
  if echo "$resp" | python -c "import json,sys; d=json.load(sys.stdin); sys.exit(0 if 'deploymentId' in d else 1)" 2>/dev/null; then
