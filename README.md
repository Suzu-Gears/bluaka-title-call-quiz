...
- `label` / `voiceActor` は R2 の `meta/audio-labels.json` で人手で付ける表示名・声優名
  （例: 「旧声優」。バッジ幅の都合で 3 文字まで）。キーは `{clipId}.g{世代}` でフォルダを含まないため、移動しても壊れません。
...
```json
{
  "cherino_title.g1": { "label": "旧声優", "voiceActor": "○○" },
  "cherino_title.g2": "現行版"
}
```

値は文字列（ラベルのみの省略記法）か `{ label, voiceActor }` オブジェクトです。
**ラベルは 3 文字までにしてください。** カード上のバッジは「2/2 ラベル ⇄」の形で
画像幅に収める必要があり、4 文字だと「旧声優…」のように省略されます。
