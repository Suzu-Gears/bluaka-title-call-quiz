...
- `label` / `voiceActor` は R2 の `meta/audio-labels.json` で人手で付ける表示名・声優名
  （「初代」「2代目」「現行」。バッジ幅の都合で 3 文字まで）。キーは `{clipId}.g{世代}` でフォルダを含まないため、移動しても壊れません。
...
```json
{
  "cherino_title.g1": { "label": "初代", "voiceActor": "○○" },
  "cherino_title.g2": "現行"
}
```
...
**ラベルの付け方は「初代 / N 代目 / 現行」で統一します。**

| 世代 | ラベル |
| ---- | ------ |
| 最初 | `初代` |
| 中間 | `2代目` `3代目` …（N は算用数字。バッジ内の「2/3」と表記を揃え、字幅も狭い） |
| 最新 | `現行` |

担当声優を「旧」と呼ばず、歴代として並べる言い方にしています。
**ラベルは 3 文字までにしてください。** カード上のバッジは「2/3 ラベル ⇄」の形で
画像幅に収める必要があり、高精細端末（カードが小さく描画される）では 4 文字から
「初代収…」のように省略されます。
