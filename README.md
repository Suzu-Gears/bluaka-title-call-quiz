...
| ケース                     | 実データ                                             | 扱い                                                           |
| -------------------------- | ---------------------------------------------------- | -------------------------------------------------------------- |
| 同名 2 レコード・音声 1 本 | ホシノ（臨戦） Id=10098 / 10099。音声は 10098 のみ   | 1 エントリへ統合。画像は両方保持、代表は音声を持つ 10098       |
| 同名 2 レコード・音声 2 本 | シュン（水着） Id=10143 / 10144。10143 に 2 クリップ | 1 エントリへ統合し、2 本とも出題候補（毎回ランダムに 1 本）    |
...
- `label` は `src/data/audioLabels.ts` で人手で付けた表示名（例: 「旧声優版」）。
...
<div class="grid-item" tabindex="0"
     ...
     data-name-sort-order="12"
     data-has-audio="true"
     data-clip-index="0">                <!-- 次にタップしたときに鳴るクリップ -->
  <div class="image-container">
    <img loading="lazy" src="image/10143.webp" alt="シュン（水着）" />
    <div class="clip-badge">1/2</div>    <!-- 複数クリップ or ラベルがある場合のみ -->
...
画像の URL は `image/{PrimaryId}.webp` です。生徒名を URL に含めないため、
日本語名のエンコード問題が発生しません。
...
また旧世代も含めて全クリップを聴けるようにし、タップのたびに順送りします（最新世代が先頭）。
